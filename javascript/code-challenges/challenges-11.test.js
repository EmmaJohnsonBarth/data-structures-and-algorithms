...
const count = (target, input) => {
  let count = 0;
  input.map(ele => ele.filter(num => num === target)).flat().forEach(() => count++);
  return count;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map(innerArr => {
    return innerArr.filter(innerEle => typeof innerEle === 'number' && innerEle % 5 === 0)
                  .map(filteredEle => (2 ** filteredEle));
  });
